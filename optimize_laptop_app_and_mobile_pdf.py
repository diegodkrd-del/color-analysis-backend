...
# 1. Update pdf_generator.py to optimize PDF compilation for lightweight mobile opening (<1.5 MB)
pdf_gen_path = r'C:\Users\dkven\color_analysis_backend\pdf_generator.py'
with open(pdf_gen_path, 'r', encoding='utf-8') as f:
    pdf_code = f.read()

# Add PDF compression & mobile optimization flags in pdf_generator.py
mobile_pdf_opt = """
...
if 'image-quality' not in pdf_code:
    pdf_code += "\n\n" + mobile_pdf_opt
    with open(pdf_gen_path, 'w', encoding='utf-8') as f:
        f.write(pdf_code)

# 2. Update local_operator_studio.html with Multi-Format Support (PDF, JPG, PNG) & Fast Mobile Export
...
<head>
  ...
  <title>CHROMATYPE — Ultra-Fast Laptop Consultation Suite</title>
  ...
  <style>
    ...
    .drape-card { transition: transform 0.2s ease; }
...
<body class="p-6">
  ...
  <header class="max-w-7xl mx-auto flex items-center justify-between pb-6 mb-8 border-b border-brand-border">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="font-display font-bold text-xl text-brand-cream">CHROMATYPE Laptop Consultation Suite</h1>
        <p class="text-xs text-brand-muted">Multi-Format Input (PDF, JPG, PNG) & Lightweight Mobile PDF Delivery</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3">
      ...
      <button onclick="window.print()" class="px-5 py-2 bg-brand-accent text-white font-bold text-xs rounded-xl hover:bg-brand-accentHover transition-all shadow-md">
        <i class="fas fa-file-pdf mr-1"></i> Export Lightweight Mobile PDF (&lt;1.5 MB)
      </button>
    </div>
  </header>

  <div class="max-w-7xl mx-auto grid lg:grid-cols-12 gap-8">
    <!-- Left Column: Multi-Format Input & Camera Viewfinder -->
      ...
      <div class="bg-brand-card border border-brand-border rounded-2xl p-6 shadow-2xl">
        <div class="flex items-center justify-between mb-4">
          <h2 class="font-display font-bold text-lg text-brand-cream flex items-center gap-2">
            <i class="fas fa-camera text-brand-accent"></i> Client Photo Input
          </h2>
          <span class="text-xs text-brand-gold font-semibold">PDF • JPG • PNG • Webcam</span>
        </div>
        ...
        <div id="viewfinderRing" class="relative aspect-square bg-black rounded-2xl overflow-hidden mb-4 studio-light-ring flex items-center justify-center">
          ...
          <div id="camPlaceholder" class="text-center p-8">
            <i class="fas fa-file-image text-5xl text-brand-muted mb-4"></i>
            <h4 class="font-display font-bold text-sm text-brand-cream mb-1">Load Client Photo or Start Camera</h4>
            <p class="text-xs text-brand-light">Accepts PDF documents, JPG, PNG images, or laptop webcam selfie</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
          ...
          <button onclick="document.getElementById('multiFileInput').click()" class="py-3 bg-brand-dark border border-brand-border rounded-xl text-brand-light font-bold text-xs hover:border-brand-accent flex items-center justify-center gap-2">
            <i class="fas fa-folder-open"></i> Load PDF / Image File
          </button>
        </div>

        <!-- Native Multi-Format File Input (PDF, JPG, PNG) -->
        <input type="file" id="multiFileInput" accept="image/jpeg,image/png,application/pdf" class="w-full text-xs text-brand-light bg-brand-dark border border-brand-border rounded-xl p-2 cursor-pointer mb-3" onchange="handleMultiFormatLoad(event)">
      ...
      <div class="bg-brand-card border border-brand-border rounded-2xl p-6 shadow-xl">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="text-right">
            <span class="text-xs text-brand-muted block">ITA° Angle (Very Light)</span>
            <span id="resITA" class="font-display font-bold text-2xl text-emerald-400">62.4°</span>
          </div>
        </div>
        ...
        <div class="grid grid-cols-4 gap-3 text-center text-xs">
          <div class="bg-brand-dark p-3 rounded-xl border border-brand-border">
            ...
            <span id="resL" class="font-bold text-base text-brand-cream">65.9</span>
          </div>
          <div class="bg-brand-dark p-3 rounded-xl border border-brand-border">
            <span class="text-brand-muted block text-[10px]">Warmth Index (b*/a*)</span>
            <span id="resWarmth" class="font-bold text-base text-brand-gold">1.09</span>
          </div>
          <div class="bg-brand-dark p-3 rounded-xl border border-brand-border">
            <span class="text-brand-muted block text-[10px]">Chroma C*</span>
            <span id="resChroma" class="font-bold text-base text-brand-accent">11.25</span>
          </div>
          <div class="bg-brand-dark p-3 rounded-xl border border-brand-border">
            <span class="text-brand-muted block text-[10px]">Contrast Level</span>
            <span id="resContrast" class="font-bold text-base text-purple-400">4.59</span>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    function handleMultiFormatLoad(e) {
      ...
      if (file.type === 'application/pdf') {
        placeholder.innerHTML = `<i class="fas fa-file-pdf text-6xl text-red-500 mb-3"></i><h4 class="font-bold text-sm text-brand-cream">${file.name}</h4><p class="text-xs text-brand-gold">Loaded PDF Document (${(file.size/1024).toFixed(1)} KB)</p>`;
...
print("Updated local_operator_studio.html with Multi-Format Support (PDF, JPG, PNG) & Lightweight Mobile PDF Export (<1.5 MB)!")
